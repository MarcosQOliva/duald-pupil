function duald_F(subjectID, subjectAge, subjectGender, F)
%DUALD_F Run dual decision task with fixed difficulty +/-F.
%   duald_F(subjectID, subjectAge, subjectGender, F) runs the task using a
%   constant numerosity difference (F) for both decisions on every trial.
%
%   The function collects the same participant info as the original script
%   (id/age/gender) but now receives them as inputs alongside the fixed
%   difficulty value F.

if nargin < 4
    error('Usage: duald_F(subjectID, subjectAge, subjectGender, F)');
end
...
% Normalize difficulty to a positive integer
totalDots = 100; % fixed total (2 * ndots_ref)
fixed_diff = round(totalDots * tanh(abs(F)/2));
if fixed_diff == 0
    warning('Provided F is too small; using a minimum difference of 1.');
    fixed_diff = 1;
end
...
% practice_min = max(1, round(0.5 * fixed_diff));
% practice_max = max(practice_min, round(1.5 * fixed_diff));
...
for t = 1:n_trials_practice
    ...
    % run trials
    %d_i = randi([practice_min, practice_max],1,2);
...
for t = 1:n_trials
    ...
    d_i = repmat(fixed_diff, 1, 2);
    [dataline1, dataline2, first_correct, second_correct] = runSingleTrial(scr, visual, leftKey, rightKey, soa_range, d_i, collect_confidence, false);
    ...
    % save data
    dataline1 = sprintf('%s\t%s\t%s\t%i\t%s\t%s\t%.5f\n', subjectID, subjectAge, subjectGender, t, dataline1, 'fixed', F);
    fprintf(datFid, dataline1);
    
    dataline2 = sprintf('%s\t%s\t%s\t%i\t%s\t%s\t%.5f\n', subjectID, subjectAge, subjectGender, t, dataline2, 'fixed', F);
...
total_score = sum(ACC);
file_name = sprintf('%s_fixed_score.txt', subjectID);
